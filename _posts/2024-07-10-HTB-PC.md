...
First let's start by running enumeration on box located at `10.10.11.214` I like to use the tool [AutoRecon](https://github.com/Tib3rius/AutoRecon) as it vastly speeds up the enumeration process
...
Doing a bit of research to get a high-level overview of this exploit tells me it's a unauthenticated RCE that takes advantage of improper handling of user inputs, specifically related to the application's processing of JavaScript. [CVE-2023-0297](https://nvd.nist.gov/vuln/detail/CVE-2023-0297)
...
[+] The exploit has be executeded in target machine.` which indicates that the script is working
...
3. Discoverd the service was interacting with an SQLite database
4. Found & exploited an SQLi in the gRPC service
5. SQLi provided SSH credentials
6. Discovered local web service running on port 
7. Used SSH port forwarding to access the local PyLoad service
8. Utilized searchsploit to discover CVE related to the PyLoad service
9. Created malicious bash script for a reverse shell
10. Exploited to PyLoad vulnerability to execute the malicious script
11. Obtained a reverse shell connect with root privileges on the target machine

### Prevention

- gRPC service securirty (Input validation & parameterized queries to prevent SQLi)
- Keep all software, especially web applications like PyLoad updated to the latest versions
- Avoid running applications as root (least privilege)
- IDS/IPS software to detect SQLi attempts